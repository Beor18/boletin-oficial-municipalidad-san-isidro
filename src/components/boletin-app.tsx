...
import type { Boletin, BoletinesPorAnio, Resolucion, Articulo } from "@/lib/types";
import {
  getBoletin,
  getBoletines,
  crearBoletin,
...
const resolucionSchema = z.object({
  ...
  tipo: z.enum(["RESOLUCIÓN", "PROMULGACIÓN", "ORDENANZA"]),
  numero: z.string().min(1, "El número es requerido"),
...
interface BoletinAppProps {
  initialBoletines: Boletin[];
  initialBoletinesPorAnio: BoletinesPorAnio;
  initialBoletinId: string | null;
}

export function BoletinApp({
  initialBoletines,
  initialBoletinesPorAnio,
  initialBoletinId,
}: BoletinAppProps) {
  // Boletines state - inicializado con datos del servidor
  const [boletines, setBoletines] = useState<Boletin[]>(initialBoletines);
  ...
  // Cargar boletines (para refrescar después de mutaciones)
  const loadBoletines = useCallback(async () => {
    const data = await getBoletines();
  ...
  const loadBoletinActual = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const data = await getBoletin(boletinSeleccionado);
  ...
  // Cargar boletín inicial si hay uno seleccionado
  useEffect(() => {
    if (initialBoletinId) {
      loadBoletinActual();
    }
  }, [initialBoletinId, loadBoletinActual]);

  // Cargar boletín cuando cambia la selección
  useEffect(() => {
    loadBoletinActual();
  }, [loadBoletinActual]);
  ...
  const handleCrearBoletin = async (data: {
    ...
    const result = await crearBoletin(data);

    if (!result.success) {
      throw new Error(result.error || "Error al crear boletín");
    }

    toast.success(
      `Boletín ${result.boletin?.mesNombre} ${result.boletin?.anio} creado correctamente`
    );
    await loadBoletines();
    if (result.boletin) {
      setBoletinSeleccionado(result.boletin.id);
    }
  };

  const handleActivarBoletin = async (id: string) => {
    const result = await activarBoletin(id);

    if (!result.success) {
      toast.error(result.error || "Error al activar el boletín");
      return;
    }
  ...
  const handleEliminarBoletin = async (id: string) => {
    const result = await eliminarBoletin(id);
  ...
  const handleCerrarBoletin = async (id: string) => {
    if (
      !confirm(
        "¿Está seguro que desea cerrar este boletín? Esta acción es irreversible y el boletín quedará archivado sin posibilidad de edición."
      )
    ) {
      return;
    }

    const result = await cerrarBoletin(id);

    if (!result.success) {
      toast.error(result.error || "Error al cerrar el boletín");
      return;
    }

    toast.success("Boletín cerrado y archivado correctamente");
    await loadBoletines();
    await loadBoletinActual();
  };
  ...
  const onSubmit = async (data: ResolucionFormValues) => {
    try {
      const payload = {
        ...
        cierre: data.cierre,
        boletinId: boletinSeleccionado || undefined,
      };

      let result;
      if (editingId) {
        result = await editarResolucion(editingId, payload);
      } else {
        result = await crearResolucion(payload);
      }
      ...
      toast.error(
        error instanceof Error ? error.message : "Error al guardar la resolución"
      );
      console.error(error);
    }
  };
  ...
  const handleEdit = (resolucion: Resolucion) => {
    ...
    form.reset({
      ...
      fecha: resolucion.fecha.split("T")[0],
      tipo: resolucion.tipo as "RESOLUCIÓN" | "PROMULGACIÓN" | "ORDENANZA",
  ...
  const handleDelete = async (id: string) => {
    const result = await eliminarResolucion(id);

    if (!result.success) {
      toast.error(result.error || "Error al eliminar la resolución");
      return;
    }
  ...
  const ResolucionCard = ({
    resolucion,
    boletinCerrado,
  }: {
    resolucion: Resolucion;
    boletinCerrado?: boolean;
  }) => {
        ...
        <main className="flex-1 overflow-auto p-6">
          {isFormVisible ? (
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-400 mt-1">
                      {(boletinActual.totalOrdenanzas || 0) +
                  ...
                  <div className="flex gap-2">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {!boletinActual.activo && !boletinActual.cerrado && (
                          <DropdownMenuItem
                            onClick={() =>
                        ...
                        {!boletinActual.activo && !boletinActual.cerrado && (
                          <DropdownMenuItem
                            onClick={() =>
                              handleCerrarBoletin(boletinActual.id)
                            }
                          >
                    ...
                    <Button
                      ...
                      disabled={
                        isGeneratingPreview ||
                        (!boletinActual.ordenanzas?.length &&
              ...
              {/* Ordenanzas */}
              {boletinActual.ordenanzas &&
                boletinActual.ordenanzas.length > 0 && (
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-3">
                      {boletinActual.ordenanzas.map((ordenanza) => (
                        <ResolucionCard
                          key={ordenanza.id}
                          resolucion={ordenanza}
                          boletinCerrado={boletinActual.cerrado}
                        />
                      ))}
                    </div>
                  </div>
                ...
                !boletinActual.resoluciones?.length && (
                  <Card className="text-center py-12">
                    <CardContent>
                      ...
                      <h3 className="text-xl font-semibold mb-2 text-slate-700">
                        {boletinActual.cerrado
                          ? "Boletín archivado vacío"
                          : "Boletín vacío"}
                      </h3>
                      <p className="text-slate-500 mb-6">
                        {boletinActual.cerrado
                          ? "Este boletín está archivado y no tiene documentos."
                          : "Este boletín aún no tiene documentos. Agregue una promulgación o resolución."}
                      </p>
                      {!boletinActual.cerrado && (
          ...
          <div className="relative w-full max-w-4xl max-h-[90vh] bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="text-right text-slate-600">
                ...
                {new Date(viewingResolucion.fecha.split("T")[0] + "T12:00:00").toLocaleDateString("es-AR", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })}
